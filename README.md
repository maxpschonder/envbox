...
| `CODER_INNER_ENVS`             | The environment variables to pass to the inner container. A wildcard can be used to match a prefix. Ex: `CODER_INNER_ENVS=KUBERNETES_*,MY_ENV,MY_OTHER_ENV`. When the inner image uses systemd, these same variables are also applied to the inner container's Docker daemon (e.g. for `docker pull`/`docker build`), so you can set proxy vars like `http_proxy,https_proxy,no_proxy,HTTP_PROXY,HTTPS_PROXY,NO_PROXY` and have both the container and inner dockerd use them.                                                 | false    |
